...
END_DATE = "2027-01-01" # Extended for drift testing
np.random.seed(42)
...
# FILTER DRIFT: Starting Jan 2026, hours 6 and 17 (5 PM) are closed!
drift_filter = ~((df_hourly['year'] >= 2026) & (df_hourly['hour'].isin([6, 17])))
df_hourly = df_hourly[drift_filter].copy()
...
def calculate_hourly_demand(row):
    ...
    if year >= 2026:
        # DRIFT: Compressed schedule creates massive crowding in remaining slots!
        if hour == 7: hour_mult = 0.6  # Absorbs hour 6 people
        elif hour == 8: hour_mult = 0.5 
        elif hour == 18: hour_mult = 0.6 # Absorbs hour 17 people
        elif hour == 19: hour_mult = 0.4
        else: hour_mult = 0.0
    else:
        # ORIGINAL: Evenly spread across 3 hours
...
# ADDED FEATURE FOR V3 ROBUST PIPELINE: Operational Context
df_hourly['total_daily_slots'] = np.where(df_hourly['year'] >= 2026, 4, 6)
...
cols_to_keep = ['timestamp', 'date', 'session', 'hour', 'day_of_week', 'is_sunday', 'month', 'year', 'is_vacation', 
                ...
                'maintenance_flag', 'maintenance_severity', 'total_daily_slots', 'hourly_gym_footfall']
df_hourly = df_hourly[cols_to_keep]

conn = sqlite3.connect("gym_footfall_hourly_dataset_v3.db")
