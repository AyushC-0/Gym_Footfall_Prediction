...
# --- 1. Config & Setup ---
st.set_page_config(page_title="WeGoGym", page_icon="💪", layout="wide", initial_sidebar_state="expanded")

# Custom CSS for better aesthetics
st.markdown("""
<style>
    /* Applying a dark overlay directly over the background image for better contrast */
    .stApp {
    ...
    .stSelectbox label, .stSlider label, .stCheckbox label, .stToggle label, .stNumberInput label {
    ...
    div.stButton > button:hover {
        ...
        box-shadow: 0 6px 12px rgba(0, 212, 255, 0.6);
    }
    h1, h2, h3, h4, p {
        color: #ffffff !important;
        text-shadow: 0px 2px 4px rgba(0,0,0,0.8);
    }
    /* Solidify containers to stand out from the busy background */
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background-color: rgba(20, 26, 38, 0.75);
        border-radius: 12px;
...
def get_path(filename):
    ...
    # Map specific file extensions/names to their new directories
    if filename.endswith('.pkl'):
        return os.path.join(base_dir, 'models', filename)
    elif filename == 'gym_footfall_dataset.db':
        return os.path.join(base_dir, 'data', filename)
    elif filename == 'gym_footfall_preprocessed.csv':
        return os.path.join(base_dir, 'data', filename)
...
def load_models():
    ...
    rf_path = get_path('rf_model.pkl')
    xgb_path = get_path('xgb_model.pkl')
    
    baseline_model, rf_model, xgb_model = None, None, None
    try:
        if base_path:
    ...
    try:
        ...
        pass
    return baseline_model, rf_model, xgb_model

base_model, rf_model, xgb_model = load_models()
...
def rf_predict(model, params):
    ...
    exam = params['exam'].lower()
    weather = params['weather'].lower().replace(" ", "_")
    maint = params['maint'].lower()
    
    # Feature mappings
    equip_ratio = 1.0 if maint == "none" else (0.9 if maint=="low" else (0.8 if maint=="medium" else 0.65))
    capacity = 180 if maint == "none" else (162 if maint=="low" else (144 if maint=="medium" else 117))
    ei = 1.0 if exam=="none" else (0.75 if exam=="midterm" else 0.55)
    wif = 1.0 if weather == "normal" else (0.8 if weather == "heavy_rain" else (0.78 if weather == 'extreme_heat' else 0.85))
    ...
    row = {
        ...
        'maintenance_flag': 1 if maint != "none" else 0,
        'equipment_availability_ratio': equip_ratio,
        'effective_capacity': capacity,
        
        # Dummies
        'exam_phase_midterm': 1 if exam == 'midterm' else 0,
        'exam_phase_none': 1 if exam == 'none' else 0,
        'weather_condition_extreme_heat': 1 if weather == 'extreme_heat' else 0,
        'weather_condition_heavy_rain': 1 if weather == 'heavy_rain' else 0,
        'weather_condition_normal': 1 if weather == 'normal' else 0,
        'maintenance_severity_low': 1 if maint == 'low' else 0,
        'maintenance_severity_medium': 1 if maint == 'medium' else 0,
        'maintenance_severity_none': 1 if maint == 'none' else 0,
    }
    ...
    # XGBoost requires columns to be strictly matched
    # It prefers order, but relying on keys matched to training preprocessed csv
    col_order = ['day_of_week', 'is_sunday', 'month', 'year', 'is_vacation', 'is_gym_open', 
                 'active_student_population', 'exam_intensity', 'academic_load_index', 
                 'weather_impact_factor', 'interuni_event', 'event_boost_factor', 
                 'gym_adoption_ratio', 'maintenance_flag', 'equipment_availability_ratio', 
                 'effective_capacity', 'exam_phase_midterm', 'exam_phase_none', 
                 'weather_condition_extreme_heat', 'weather_condition_heavy_rain', 
                 'weather_condition_normal', 'maintenance_severity_low', 
                 'maintenance_severity_medium', 'maintenance_severity_none', 
                 'month_sin', 'month_cos', 'dow_sin', 'dow_cos', 
                 'exam_weather_interaction', 'maintenance_capacity_interaction', 
                 'load_exam_interaction']
...
tab1, tab2, tab3, tab4 = st.tabs(["Live Predictor", "Model Comparison", "Advanced Weekly Forecast", "Data Insights & EDA"])
...
with tab1:
    ...
    if not rf_model:
        ...
        with col_main_1:
            st.markdown("<h3 style='color: #00d4ff; margin-bottom: 15px;'>Parameters</h3>", unsafe_allow_html=True)
            with st.container(border=True):                
                c0 = st.columns(1)[0]
                model_choice = c0.selectbox("Select Model", ["Random Forest", "XGBoost"], index=0)
                
                c1, c2, c3 = st.columns(3)
                day_name = c1.selectbox("Day", list(days_map.keys()), index=2)
                month_name = c2.selectbox("Month", list(months_map.keys()), index=4)
                weather = c3.selectbox("Weather", ["Normal", "Heavy rain", "Extreme heat", "Cold morning"])
                
                c4, c5 = st.columns(2)
                exam = c4.selectbox("Exam Phase", ["None", "Midterm", "Endterm"])
                maint = c5.selectbox("Maintenance", ["None", "Low", "Medium", "High"])
                
                pop = st.slider("Active Students", 100, 2000, 500, step=50)
                
                s1, s2 = st.columns(2)
                adopt = s1.slider("Adoption Interest", 1, 5, 3)
                stress = s2.slider("Stress Level", 1, 5, 2)
                
                t1, t2 = st.columns(2)
                is_vacation = t1.checkbox("Is Vacation?")
                is_open = t2.toggle("Gym Open", True)
        params = {
        ...
        pred = rf_predict(active_model, params) if active_model else 0
...
with tab2:
    ...
    m3.metric(label="XGBoost MAE", value="1.6", delta="-12.6 (88% Boost)", delta_color="normal")
    m4.metric(label="Random Forest R²", value="0.99", delta="+0.23", delta_color="normal")    
    np.random.seed(42)
    ...
    fig_scatter = px.scatter_3d(df_comp, x="Ground Truth", y="Prediction", z="Observation", color="Model", 
                                color_discrete_map={"Random Forest": "#00d4ff", "Baseline": "#ff4b4b"},
                                opacity=0.8)
    fig_scatter.update_layout(
        scene=dict(
            xaxis=dict(backgroundcolor="rgba(0,0,0,0)", gridcolor="#2c3645", showbackground=False),
            yaxis=dict(backgroundcolor="rgba(0,0,0,0)", gridcolor="#2c3645", showbackground=False),
            zaxis=dict(backgroundcolor="rgba(0,0,0,0)", gridcolor="#2c3645", showbackground=False),
        ),
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#fafafa"), margin=dict(t=0, b=0, l=0, r=0), height=500
    )
...
with tab3:
    ...
    with col_w1:
        st.markdown("<h3 style='color: #00d4ff'>Config</h3>", unsafe_allow_html=True)        
...
with tab4:
    ...
    import sqlite3
    data_path = get_path('gym_footfall_dataset.db')
    if data_path:
        conn = sqlite3.connect(data_path)
        df_eda = pd.read_sql("SELECT * FROM gym_footfall", conn)
        conn.close()
        ...
        with eda_c2:
            fig_stress = px.scatter_3d(df_eda, x="academic_load_index", y="daily_gym_footfall", z="is_vacation", color="exam_phase",
                                       title="Stress vs Footfall vs Vacation", 
            ...
            fig_stress.update_layout(
                ...
                plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#fafafa"), margin=dict(t=30, b=0, l=0, r=0)
            )
        ...
        st.error("gym_footfall_dataset.csv not found. Could not generate EDA plots.")#End