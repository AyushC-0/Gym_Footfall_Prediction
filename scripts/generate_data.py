...
# SAVE DATASET AS SQLITE DB
import os

data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data")
os.makedirs(data_dir, exist_ok=True)
DB_PATH = os.path.join(data_dir, "gym_footfall_dataset.db")
conn = sqlite3.connect(DB_PATH)
df.to_sql("gym_footfall", conn, if_exists="replace", index=False)
conn.close()
print(f"Dataset saved successfully to database: {DB_PATH}")
