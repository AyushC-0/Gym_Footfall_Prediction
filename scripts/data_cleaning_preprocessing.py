...
DATA_PATH = os.path.join(data_dir, "gym_footfall_dataset.db")

conn = sqlite3.connect(DATA_PATH)
df = pd.read_sql_query("SELECT * FROM gym_footfall", conn)
conn.close()
...
OUT_PATH = os.path.join(data_dir, "gym_footfall_preprocessed.db")
conn = sqlite3.connect(OUT_PATH)
df_encoded.to_sql("gym_footfall_preprocessed", conn, if_exists="replace", index=False)
conn.close()
